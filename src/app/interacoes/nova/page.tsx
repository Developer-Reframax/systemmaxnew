...
export default function NovaInteracao() {
  ...
  const loadDropdownData = async () => {
    try {
      ...
      const [
        ...
        fetch('/api/interacoes/violacoes', { headers }),
        fetch('/api/interacoes/grandes-riscos', { headers }),
        fetch('/api/interacoes/local-instalacao', { headers }),
        fetch('/api/security-params/locations', { headers }),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Obter matrícula do usuário autenticado do token
      let matriculaUsuario = ''
      if (auth_token) {
        try {
          const payload = JSON.parse(atob(auth_token.split('.')[1]))
          matriculaUsuario = payload.matricula?.toString() || ''
        } catch (error) {
      ...
      const formDataWithColaborador = {
        ...restFormData,
        matricula_colaborador: matriculaUsuario,
