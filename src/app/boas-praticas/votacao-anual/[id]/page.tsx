...
export default function VotacaoAnualDetalhe() {
  useAuth()
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await fetch(`/api/boas-praticas/votacao-anual/${params.id}`, {
        ...
        router.push('/boas-praticas/votacao-anual')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/boas-praticas/votacao-anual/${pratica.id}`, {
        method: 'POST',
        headers: {
    ...
    <MainLayout>
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" onClick={() => router.push('/boas-praticas/votacao-anual')}>
            Voltar
          </Button>
        </div>
