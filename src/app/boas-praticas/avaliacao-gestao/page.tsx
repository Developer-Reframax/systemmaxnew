...
export default function AvaliacoesGestaoPage() {
  useAuth()
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await fetch('/api/boas-praticas/avaliacoes-gestao', { headers: { Authorization: `Bearer ${token}` } })
    ...
    <MainLayout>
      <div className="container mx-auto p-6 space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Avaliar Boas Praticas (Gestao)</h1>
            <p className="text-gray-600">Praticas aguardando avaliacao da gestao</p>
          </div>
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="space-y-3">
                {items.map(item => (
                  <Card key={item.id} className="hover:shadow-lg transition-shadow border border-purple-100">
                    <CardContent className="flex items-center justify-between p-4">
                      <div className="flex flex-col">
                        <span className="font-semibold text-gray-900">{item.titulo}</span>
                        <span className="text-sm text-gray-700">Contrato: <span className="font-medium text-purple-700">{item.contrato || '-'}</span></span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => router.push(`/boas-praticas/avaliacao-gestao/${item.id}`)}
                        >
                          Realizar avaliacao
                        </Button>
                      </div>
