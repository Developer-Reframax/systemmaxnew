...
// GET /api/interacoes/unidades - Listar unidades
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from('interacao_unidades')
      .select('id, unidade, contrato_id, created_at, updated_at')
      .eq('contrato_id', contratoFiltro)
      .order('unidade')

    if (search) {
      query = query.ilike('unidade', `%${search}%`)
    }
...
// POST /api/interacoes/unidades - Criar nova unidade
...
// PUT /api/interacoes/unidades - Atualizar unidade
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json()
    const { id, unidade } = body

    if (!id || !unidade) {
      return NextResponse.json(
        { error: 'ID e unidade são obrigatórios' },
        { status: 400 }
      )
    }
...
// DELETE /api/interacoes/unidades - Deletar unidade
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: { message: 'Unidade deletada com sucesso' } })
  } catch (error) {
    console.error('Erro interno:', error)
    return NextResponse.json(
      { error: 'Erro interno do servidor' },
